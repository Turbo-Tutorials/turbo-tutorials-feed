...
export function enhanceContentfulItem(item: any) {
...
function transformContentfulFields(entry: any) {
  if (entry) {
    ...
    Object.keys(content).map((fieldKey) => {
      if (
        ...
        const transformedImages = content[fieldKey].map((asset: any) =>
          transformContentfulImage(asset)
        );
        content[fieldKey] = transformedImages;
      } else if (Array.isArray(content[fieldKey])) {
        const flattenedFields = content[fieldKey].map((entry: any) => {
          return { ...entry.fields };
        });
...
function transformContentfulImage(imageField: any) {
